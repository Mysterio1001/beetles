...
@mixin circle($size: 7rem, $color: color(white)) {
  height: $size;
  width: $size;
...
.headerSticky {
  ...
  .header {
    ...
    padding: 0 1rem;
    height: 10rem;
    background: linear-gradient(to right, color(green-02), color(green-01));
    opacity: 1;
    ...
    ul {
      ...
      li {
        ...
        @include md {
          ...
          border-bottom: 1px solid color(green-04);
    ...
    .shoppingCartRwd {
      ...
      @include md {
        display: flex;
        @include circle($color: color(green-03));
      }
    }
  }
}
